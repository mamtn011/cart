...
class UI {
    ...
    async #getApiData() {
        const product = await (0, _productJsDefault.default).getProduct();
    ...
    #getFilterdProducts(category) {
        let products = [];
        if (!category) products = this.#products;
        else products = this.#products.filter((product)=>category.id === product.categoryId);
        return products;
    }
    #showProductToUI(categoryName = "All") {
        ...
        const category1 = this.#getCategoryId(categoryName);
        const products1 = this.#getFilterdProducts(category1);
        let elm = "";
    ...
    showStorageDataToUI() {
        const { cartContentElm , continueElm , cartPriceElm , cartQtyElm  } = ui.selectDomElm();
        cartContentElm.textContent = "";
        const products = (0, _storageJsDefault.default).getStorageData();
        let filterd = [];
        let elm = "";
        if (products.length > 0) {
            ...
            for(let i in obj)filterd.push(products.find((el)=>el.id === Number(i)));
        ...
        } else elm = `<h2 style="text-align: center">No product to checkout!</h2>`;
    ...
    async init() {
        ...
        categoryElm.addEventListener("change", ()=>this.#filterByCategory(categoryElm));
        (0, _cartJsDefault.default).init();
    }
}
const ui = new UI();
exports.default = ui;

},{"./Product.js":"btosK","./Cart.js":"1iaGP","./Storage.js":"5r0fU","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"btosK":[function(require,module,exports) {
...
},{}],"1iaGP":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _uiJs = require("./Ui.js");
var _uiJsDefault = parcelHelpers.interopDefault(_uiJs);
...
class Cart {
    #incrementCart;
    #getProduct(id) {
        return (0, _uiJsDefault.default).protucts.find((product)=>id === product.id);
    }
    #setIncrementCart() {
        ...
        } else this.#incrementCart = 0;
    }
    #getDomElm() {
        const { productsGridElm , cartCountElm , cartIcon , modalContentElm  } = (0, _uiJsDefault.default).selectDomElm();
        return {
            productsGridElm,
            cartCountElm,
            cartIcon,
    ...
    #getId(evt) {
        if (evt.target.classList.contains("cart-btn")) return Number(evt.target.parentElement.dataset.id);
    }
    #incrementCartToUI() {
        const { cartCountElm: cartCountElm1  } = this.#getDomElm();
        this.#incrementCart++;
        cartCountElm1.textContent = this.#incrementCart;
    }
    #getCartProductId(target) {
        const id1 = Number(target.parentElement.dataset.id);
        return id1;
    }
    #displayModalBlock(target1) {
        target1.parentElement.parentElement.style.display = "none";
    }
    #manipulateCart() {
        (0, _uiJsDefault.default).showStorageDataToUI();
        (0, _uiJsDefault.default).showCartCountAfterManipulate();
        this.#setIncrementCart();
    }
    #handleCartProducts(evt1) {
        if (evt1.target.classList.contains("continue-shopping")) this.#displayModalBlock(evt1.target);
        else if (evt1.target.classList.contains("modal-delete")) {
            const id2 = this.#getCartProductId(evt1.target);
            (0, _storageJsDefault.default).removeStorageData(id2);
            this.#manipulateCart();
        } else if (evt1.target.classList.contains("modal-inc")) {
            const id3 = this.#getCartProductId(evt1.target);
            (0, _storageJsDefault.default).increaseStorageData(id3);
            this.#manipulateCart();
        } else if (evt1.target.classList.contains("modal-dec")) {
            const id4 = this.#getCartProductId(evt1.target);
            (0, _storageJsDefault.default).decreaseStorageData(id4);
            this.#manipulateCart();
        }
    }
    init() {
        const { productsGridElm , cartCountElm , cartIcon , modalContentElm  } = this.#getDomElm();
        this.#setIncrementCart();
        ...
        productsGridElm.addEventListener("click", (evt)=>{
            ...
            if (id) {
                ...
                (0, _storageJsDefault.default).saveToStorage(product);
            }
        });
        cartIcon.addEventListener("click", (0, _uiJsDefault.default).showStorageDataToUI);
        modalContentElm.addEventListener("click", (evt)=>this.#handleCartProducts(evt));
    }
}
const cart = new Cart();
...
class Storage {
    ...
    getStorageData() {
        if (localStorage.getItem("myCartProducts")) return JSON.parse(localStorage.getItem("myCartProducts"));
        else return;
    }
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["1JEHZ","adjPd"], "adjPd", "parcelRequire7cbb")
